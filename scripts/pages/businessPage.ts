...
import MyPageDesign from 'generated/pages/businessPage';
...
export default class BusinessPage extends withDismissAndBackButton(MyPageDesign) {
    flHeader: FlHeader;
    data: INews[] = [];
    ...
    openNews (url: String): void {
        this.router.push('/btb/tab2/newsDetail', { url });
    }
    ...
    async getNewsData() {
        try {
            showWaitDialog()
            const response = await getNews(Category.BUSINESS, this.page);
