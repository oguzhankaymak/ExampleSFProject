...
import MyPageDesign from 'generated/pages/technologyPage';
...
export default class TechnologyPage extends withDismissAndBackButton(MyPageDesign) {
    flHeader: FlHeader;
    data: INews[] = [];
    ...
    openNews (url: String): void {
        this.router.push('/btb/tab4/newsDetail', { url });
    }
    ...
    async getNewsData() {
        try {
            showWaitDialog()
            const response = await getNews(Category.TECHNOLOGY, this.page);
