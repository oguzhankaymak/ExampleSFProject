...
import MyPageDesign from 'generated/pages/healthPage';
...
export default class HealthPage extends withDismissAndBackButton(MyPageDesign) {
    flHeader: FlHeader;
    data: INews[] = [];
    ...
    async getNewsData() {
        try {
            showWaitDialog()
            const response = await getNews(Category.HEALTH, this.page);
